...
@app.put("/items/special/{item_id}")
def update_item(item_id: int, item: Item):
    if item_id not in advanced_items:
        raise HTTPException(status_code=404, detail="Item not found")
    else:
        advanced_items[item_id] = item
        return {"current_items": advanced_items}


# Delete item endpoint
@app.delete("/items/special/{item_id}")
def delete_item(item_id: int):
    if item_id not in advanced_items:
        raise HTTPException(status_code=404, detail="Item not found")
    else:
        del advanced_items[item_id]
        return {"current_items": advanced_items}
